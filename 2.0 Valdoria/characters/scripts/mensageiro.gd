...
@onready var npc_animation: AnimationPlayer = get_node_or_null("Animation")
...
func _on_chat_detection_body_entered(body):
	if body.is_in_group("player") or body.is_in_group("character"):
		show_dialogue("Olá meu nome é Marry, sabia que o Slime Assassino não tem órgãos? Ele é basicamente um estômago gigante com sede de sangue. Fascinante, não?")
